...
def test_invalid_time_it_valueerror():
    with pytest.raises(ValueError):
        session5.time_it(session5.speed_converter,100,dist='m',time='s',repetitions=-1)  #check for invalid repetitions
    with pytest.raises(ValueError):
        session5.time_it(session5.speed_converter,100,dist='m',time='s',repetitions=5.5) #check for invalid repetitions
    with pytest.raises(ValueError):
        session5.time_it(session5.speed_converter,100,dist='m',time='s',repetitions='a') #check for invalid repetitions

def test_invalid_time_it_typeerror():
    with pytest.raises(TypeError):
        session5.time_it(session5.speed_converter,100,repetitions=1)  #check for count of arguments

def test_invalid_time_it_wrongarguments():
    with pytest.raises(TypeError):
        session5.time_it(session5.speed_converter,100,sides=3,time='s',repetitions=1)  #check for wrong arguments
    with pytest.raises(TypeError):
        session5.time_it(session5.squared_power_list,2, start1=1, end=10, repetitions=5)  #check for wrong arguments
...
def test_invalid_squared_power_list_valueerror():
    with pytest.raises(ValueError):
        session5.squared_power_list(10, -1, 'abc')   # check for invalid end parameter
    with pytest.raises(ValueError):
        session5.squared_power_list(10, 'abc', 2)    # check for invalid start parameter
...
def test_invalid_temp_converter_valueerror():
    with pytest.raises(ValueError):
        session5.temp_converter(10, 'd')       # check for invalid temp type
    with pytest.raises(ValueError):
        session5.temp_converter('abc', 'c')     # check for invalid temp
...
def test_invalid_speed_converter_valueerror():
    with pytest.raises(ValueError):
        session5.speed_converter(10, dist='d',time='s')        # check for invalid dist type
    with pytest.raises(ValueError):
        session5.speed_converter(10, dist='km',time='year')    # check for invalid time type
    with pytest.raises(ValueError):
        session5.speed_converter('abc', dist='km',time='min')  # check for invalid speed
    with pytest.raises(ValueError):
        session5.speed_converter(-10, dist='km',time='min')    # check for negative speed
